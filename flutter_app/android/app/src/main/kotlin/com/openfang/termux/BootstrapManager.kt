...
class BootstrapManager(
    ...
    fun setupDirectories() {
        listOf(rootfsDir, tmpDir, homeDir, configDir, "$homeDir/.OpenFang", libDir).forEach {
            File(it).mkdirs()
        }
    ...
    fun isBootstrapComplete(): Boolean {
        ...
        val bypass = File("$rootfsDir/root/.OpenFang/bionic-bypass.js")
        val node = File("$rootfsDir/usr/local/bin/node")
        val OpenFang = File("$rootfsDir/usr/local/lib/node_modules/OpenFang/package.json")
        return rootfs.exists() && binBash.exists() && bypass.exists()
            && node.exists() && OpenFang.exists()
    }
    ...
    fun getBootstrapStatus(): Map<String, Any> {
        ...
        val OpenFangExists = File("$rootfsDir/usr/local/lib/node_modules/OpenFang/package.json").exists()
        val bypassExists = File("$rootfsDir/root/.OpenFang/bionic-bypass.js").exists()

        return mapOf(
            "rootfsExists" to rootfsExists,
            "binBashExists" to binBashExists,
            "nodeInstalled" to nodeExists,
            "OpenFangInstalled" to OpenFangExists,
            "bypassInstalled" to bypassExists,
            "rootfsPath" to rootfsDir,
            "complete" to (rootfsExists && binBashExists && bypassExists
                && nodeExists && OpenFangExists)
        )
    }
    ...
    private fun configureRootfs() {
        ...
        aptConfDir.mkdirs()
        File(aptConfDir, "01-OpenFang-proot").writeText(
        ...
        dpkgConfDir.mkdirs()
        File(dpkgConfDir, "01-OpenFang-proot").writeText(
            "force-unsafe-io\n" +
            "no-debsig\n" +
            "force-overwrite\n" +
            "force-depends\n"
        ...
        listOf(
            ...
            // OpenFang runtime directories (can't mkdir at runtime)
            "$rootfsDir/root/.OpenFang",
            "$rootfsDir/root/.OpenFang/data",
            "$rootfsDir/root/.OpenFang/memory",
            "$rootfsDir/root/.OpenFang/skills",
            "$rootfsDir/root/.OpenFang/config",
            "$rootfsDir/root/.OpenFang/extensions",
            "$rootfsDir/root/.OpenFang/logs",
            "$rootfsDir/root/.config/OpenFang",
            "$rootfsDir/root/.local/share",
            "$rootfsDir/root/.cache",
            "$rootfsDir/root/.cache/OpenFang",
    ...
    fun createBinWrappers(packageName: String) {
        ...
        // "bin": "cli.js"  OR  "bin": {"OpenFang": "bin/OpenFang.js", ...}
    ...
    fun installBionicBypass() {
        val bypassDir = File("$rootfsDir/root/.OpenFang")
        bypassDir.mkdirs()
        ...
        val cwdFixContent = """
// OpenFang CWD Fix - Auto-generated
...
""".trimIndent()
        ...
        //    Usage: node /root/.OpenFang/node-wrapper.js <script> [args...]
        val wrapperContent = """
// OpenFang Node Wrapper - Auto-generated
...
require('/root/.OpenFang/proot-compat.js');
...
// OpenFang Proot Compatibility Layer - Auto-generated
...
""".trimIndent()
        ...
        // 4. Bionic bypass — comprehensive runtime patcher for OpenFang.
        //    Loaded via NODE_OPTIONS="--require /root/.OpenFang/bionic-bypass.js"
        val bypassContent = """
...
""".trimIndent()
        ...
        val exportLine = "export NODE_OPTIONS=\"--require /root/.OpenFang/bionic-bypass.js\""

        val existing = if (bashrc.exists()) bashrc.readText() else ""
        if (!existing.contains("bionic-bypass")) {
            bashrc.appendText("\n# OpenFang Bionic Bypass\n$exportLine\n")
        }
    }
    ...
    /** Read a file from inside the rootfs (e.g. /root/.OpenFang/OpenFang.json). */
